...
func puzzleTestFileName(year, day int) string {
	path := puzzlePath(year)
	return fmt.Sprintf("%s/day%s_test.go", path, FormatDay(day))
}
...
func puzzleProblemPath(year int) string {
	return fmt.Sprintf("./pkg/year%d/problems", year)
}

func puzzleProblemFileName(year, day int) string {
	path := puzzleProblemPath(year)
	return fmt.Sprintf("%s/%d.md", path, day)
}
...
func NewProblemFile(year, day int) {
	path := puzzleProblemPath(year)
	filename := puzzleProblemFileName(year, day)

	if _, err := os.Stat(filename); err != nil && err == os.ErrNotExist {
		logrus.Infof("Problem file already exists: %s", filename)
		return
	}
	...
	logrus.Infof("Generated empty problem file: %s", filename)

	problem, err := GetProblem(year, day)

	if err != nil {
		logrus.Fatal(err)
	}

	_, err = f.WriteString(problem)
	if err != nil {
		logrus.Fatal(err)
	}
}
...
const puzzleTestTemplate = `// Code generated by aocgen; DO NOT EDIT.
	package year{{.Year}}
	...
	func TestDay{{.Day}}(t *testing.T) {
		...
		t.Run("partA", func(t *testing.T) {
			result := day{{.Day}}.PartA(input)

			assert.Equal(t, "implement_me", result)
		})

		t.Run("partB", func(t *testing.T) {
			result := day{{.Day}}.PartB(input)
			assert.Equal(t, "implement_me", result)
		})
	}
`

func newFile(fileName string, templateContent string, templateVars map[string]any) {
	if _, err := os.Stat(fileName); err == nil || !errors.Is(err, os.ErrNotExist) {
		logrus.Infof("Puzzle file already exists: %s", fileName)
		return
	}

	tmpl := codegen.MustParse(templateContent)
	if err := tmpl.CreateFile(fileName, templateVars); err != nil {
		logrus.Fatal(err)
	}
...
func NewPuzzleTestFile(year, day int) {
	fileName := puzzleTestFileName(year, day)
	templateVars := map[string]any{
		"Year": year,
		"Day":  FormatDay(day),
	}
	newFile(fileName, puzzleTestTemplate, templateVars)
}
...
func NewPuzzleFile(year, day int) {
	...
	newFile(fileName, puzzleTemplate, templateVars)
}
